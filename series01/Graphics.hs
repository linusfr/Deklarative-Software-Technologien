...
styleToAttr (Style s) = "style=\"stroke:" ++ toLower s ++ "; fill:" ++ toLower s ++ "\""
...
formToSVG (Rectangle (Point x1 y1) (Point x2 y2) style) =  "<rect x=\"" ++ show x1 ++ "\" y=\"" ++ show y1 ++ "\" width=\"" ++ 
    show x2 ++ "\" height=\"" ++ show y2 ++ "\" " ++ styleToAttr style ++ "/>"
formToSVG (Circle (Point x1 y1) radius style) =  "<circle cx=\"" ++ show (x1+1) ++ "\" cy=\"" ++ show (y1+1) ++ "\" r=\"" ++ 
    show radius ++ "\" " ++ styleToAttr style ++ "/>"
...
toSVG (Circle (Point x1 y1) radius (Style style)) = "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"" ++
    show (radius*2+2) ++ "\" height=\"" ++ show (radius*2+2) ++ "\">\n\t" ++
